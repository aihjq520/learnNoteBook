...
需求： 告警产生时，需要播报语音和弹出消息message, 并且防止告警风暴的产生，我们只需要播报最新的5条。消息框也是同时最多同时存在5条，语音播报完成后弹框消失，自动播放下一条。也可以主动关掉弹框并且语音播报也同时消失，自动播放下一条。
...
由于弹窗组件有提供限制最大数量的属性，所以这个比较好解决。


- 难点2. **语音的播放**

播放功能调用了浏览器的api实现，出了设计方案，文档。由后端，产品评审，没有采用tts sdk等。部署成本过高。
一般的话就是说弹一个窗就播一个语音，但这里主要有两个问题， 一是由于我们的语音是分为audio, speech的，会同时播放。二是无法做到需求中的手动关闭弹窗，语音也要关掉，因为浏览器的api，cancel后会把取消所有语音。 所以我们需要同时创建一个播放的语音队列。
...
因为有个待播放的语音队列，我们可以通过id对比，如果在队列里面说明是，说明是待播放，此时需要从队列里删除
如果不在队列，说明是正在播放，需要暂停语音。

2. 语音播放完，弹窗消失

所幸的是所有的语音都有提供结束事件的回调，所以我们在播放语音的时候，从待播放队列取取出语音播放的时候，设置结束函数，因为有存储message的引用，可以调用close。
...
调试困难， 查看英文doc， 解决了代理问题

TS + class 写法 （class写法不太熟悉， 按照之前function写法）
